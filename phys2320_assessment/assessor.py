...
from .funcs import open_figures, isiterable, touch, read_user_data, raiseExit,  parse_code, is_mod_function
...
####################################################################################
    ...
    def save_figs(self,pattern,title,prefix="<h3>Figures from {}</h3>"):
        ...
        try:
            for i,fig in enumerate(open_figures()): # Close and open figures from the import
                ...
                out.append("<td><a href='{filename}'><img src='{filename}' width=200px></a></td>".format(filename=filename))
    ...
    def three_way(self,s,m,c):
        ...
        if student_correct and model_correct and student_model:
            ret="Student and model agree and are correct within {} std errors".format(m.margin),0
        elif student_correct and model_correct and not student_model:
            ret="Both student and model are correct within {} std errors but don't match".format(m.margin),0
        elif student_correct and not model_correct:
            ret="Student correct within {} std errors, but model answer isn't".format(m.margin),1
        elif model_correct and not student_correct:
            ret="Model answer correct within {} std errors, but student is not".format(m.margin),2
        elif student_model and not student_correct and not model_correct:
            ret="Student and model answer agree within {} std errors but are not correct".format(m.margin),3
        ...
        if isinstance(model_ans,list):#This is a list of answers, so we need to recurse through it.
            if isinstance(student,(list,np.ndarray)) and len(student)<=len(model_ans):
                ...
                for i,sa in enumerate(student):
                    self.compare_one_val(sa,model_ans[i],calc_ans[i],"{}[{}]".format(key,i))
            elif isinstance(student,(list,np.ndarray)):
                print("<tr><td colpan=5>Answer is a list with more items than the model answer for {}</td></tr>".format(key))
                for i,ma in enumerate(model_ans):
                    self.compare_one_val(student[i],ma,calc_ans[i],"{}[{}]".format(key,i))
            else:
            ...
            self.compare_dict(student,model_ans,calc_ans,header="Comapring sub-dictionary for {}".format(key))
            ...
            try:
                ...
                raise ValueError(repr(err)+"\n"+key+"\nCalc:"+repr(calc_ans)+"\nStudent:"+repr(student)+"\nModel:"+repr(model_ans))
            message,score=self.three_way(student,model_ans,cval)
            klass=self.colors[score]
            print("<tr><td>{}</td><td>{}</td><td>{}</td><td>{}</td><td bgcolor='{}'>{}</td>".format(key,student,model_ans,cval,klass,message))
        else:
            print("<tr><td>{}</td><td colspan=4>Model answer not interpretable:{}</td></tr>".format(key,type(model_ans)))
    ...
    def compare_dict(self,student,model_ans,calc_ans,header=None):
        ...
        if not isinstance(student,dict):
            print("<tr><td colsp[an=5>Student code didn't return a dictionary of results - unable to process further</td></tr>")
            return
        ...
        for k in keys:
            ...
            if isinstance(calc_ans[k],list):
                if not isinstance(model_ans[k],list):
                    print("<tr><td>{}</td><td colsp[an=4>Model answer not a list - refernce code failure</td></tr>".format(k))
                    continue
                if not isinstance(student[k],(list,tuple)):
                    print("<tr><td>{}</td><td colsp[an=4>Student answer was noty a list<br/>\n{}</td></tr>".format(k,student[k]))
                    continue
    ...
    def compare(self,student,model_ans,calc_ans):
        ...
        print("<table><tr><th>Parameter</th><th>Student Answer</th><th>Model Answer</th><th>Actual Answer</th><th>Comment</th></tr>")
    ...
    def get_info(self):
        ...
        self.code=None
        self.files=[]
        with open(path.join(self.subdir,"readme.txt"),"r",encoding="utf-8",errors='backslashreplace') as readme:
    ...
    def test(self):
        ...
        with open(path.join(self.subdir,"results.html"),"w") as tmp: #sys.stdout:
            try:
                ...
                print("</table>")
                if user_settings["mode"]!="assessment":
                ...
                if self.module is None:
                    ...
                    if run_1_sresult==sresults:
                        ...
                        print("""<p>The student's solutions for their data file and the reference data file appear to
    ...
    def get_func_details(self):
        ...
        for c in classes: # This works out if we have any classes in the file
            ...
            for m in methods:
                ...
                print("<tr><td>{}</td><td>{}</td><td>{}</td></tr>".format(
                    c.__name__+"."+m.__name__,len(doc),crc32(m.__code__.co_code)))
                listing.append({
                "issid":issid,
        ...
        for m in modules:
            ...
            for f in methods:
                if f.__doc__ is not None:
                    ...
                    doc=""
                print("<tr><td>{}</td><td>{}</td><td>{}</td></tr>".format(
                    m.__name__+"."+f.__name__,len(doc),crc32(m.__code__.co_code)))
                listing.append({
                "issid":issid,
        ...
        for m in funcs:
            if m.__name__=="ProcessData": # Since students like defining functions inside ProcessData we'll have a look at them....
                reps=[]
                    ...
                    try:
                        if iscode(c):
                            ...
                            if subf.__doc__ is not None:
                                ...
                                doc=""
                            reps.append("<tr><td>{}</td><td>{}</td><td>{}</td></tr>".format(subf.__name__,len(doc),crc32(subf.__code__.co_code)))
                            listing.append({
                            "issid":issid,
                            "name":subf.__name__,
                            "docstring":doc,
    ...
    def inspect(self):
        ...
        try:
            ...
            print("""<p><b>input</b> found in student code. Using <b>input</b> will raise an error but we can try importing anyway! Students were told
    ...
    def do_import(self):
        ...
        try:
            ...
            if len(open_figures())>0:
                self.save_figs("Import_figures-{}.png","",prefix="<h3>Figures Incorrectly generated during import{}</h3>")
            ...
            print("Failed to inmport  module correctly\nmessage was:\n{}\n Check manually.".format(err).replace("\n","<br/<\n"))
            print(format_exc().replace("\n","<br/>\n"))
            self.module=None
        except Exception as err:
            print("<h2>Something is wrong with the  module\nMessage was\n{}\n Check manually !</h2>".format(err).replace("\n","<br>\n"))
            print(format_exc())
            self.module=None
        finally:
            os.chdir(back)
